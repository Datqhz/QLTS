...
public final class FoodList extends ManagerView {
    ...
    private List<SanPham> Foodlist = new ArrayList<>();
    private List<LoaiMon> listCategory = new ArrayList<>();
    FoodPopup form;

    public FoodList() {//FoodPopup form
        super();
        setHeaderTable("ID Món", "Tên món", "Đơn vị tính", "Size","Giá", "Mô tả", "Loại");
        loadData();
//        this.form = form;
...
//            listCategory = loaiMonDao.findAll();
    ...
    private void loadData() {
        ...
        for (SanPham monAn : Foodlist) {
            Object[] row = new Object[]{monAn.getIdSanPham(), monAn.getTen(),monAn.getDonViTinh(),monAn.getSize(), monAn.getGia(), monAn.getMoTa(), monAn.getTenLoai()};
            tblModel.addRow(row);
        }
